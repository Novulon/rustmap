export async function onRequest(context) {
  ...
  // 2. Sanitize special characters (| [ ] ( ) / : - _) & tokenize
  ...
  // Common high-frequency tags to deprioritize when selecting BattleMetrics anchor term
  const stopWords = new Set(['eu', 'us', 'uk', 'au', '1x', '2x', '3x', '5x', 'vanilla', 'monthly', 'weekly', 'solo', 'duo', 'trio', 'quad', 'main']);

  // 3. Select best anchor term (longest non-stopwords first)
  ...
  const bmUrl = `https://api.battlemetrics.com/servers?filter[game]=rust&filter[search]=${encodeURIComponent(anchorTerm)}&page[size]=100`;
  ...
  // 5. Response handling with conditional caching
  if (results.length > 0) {
  ...
  // Do NOT cache empty result sets in the browser
